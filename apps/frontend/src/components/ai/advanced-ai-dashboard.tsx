...
export default function AdvancedAIDashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const healthResponse = await fetch('/api/v1/ai/relationship-health');
      ...
      const stressResponse = await fetch('/api/v1/ai/team-stress');
      ...
      // Load active alerts
      const alertsResponse = await fetch('/api/v1/ai/active-alerts');
      ...
      // Load email forecast
      const forecastResponse = await fetch('/api/v1/ai/forecast-volume?period=30d');
