...
export function ProductionDashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // Load email metrics
      const metricsResponse = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/analytics/overview`);
      ...
      // Load AI insights
      const insightsResponse = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/ai/insights`);
      ...
      const relationshipsResponse = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/analytics/relationships`);
