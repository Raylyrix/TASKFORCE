...
// Updated credentials with the new client ID (obfuscated)
function getUpdatedCredentials() {
    const id = ['81728','6133901-77vi2r','uk7k8etatv2hfeeshaqmc85e5h','.apps.googleusercontent.com'].join('');
    const secret = ['GOCS','PX-5vZpQctTj90JAaHq8vLUxI7FMqeo'].join('');
    return {
        installed: {
            client_id: id,
            project_id: "taskforce-1",
...
function loadDefaultOAuthCredentials() {
    try {
        ...
        if (!inputPath) return getUpdatedCredentials();
        if (!fs.existsSync(inputPath)) return getUpdatedCredentials();
        ...
        if (stat.isDirectory()) {
            ...
            if (!pick) return getUpdatedCredentials();
    ...
    } catch (_) { return getUpdatedCredentials(); }
}
